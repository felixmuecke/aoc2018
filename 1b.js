  ...
  while (!foundDuplicate) {
    operationsArray.forEach(operation => {
      if(operation.charAt(0) === '+') {
        frequency += Number.parseInt(operation.slice(1,operation.length))
      }
      else {
        frequency -= Number.parseInt(operation.slice(1,operation.length))
      }
      let sizeBefore = frequencies.size
      frequencies.add(frequency)
      let sizeAfter = frequencies.size
      if (sizeBefore === sizeAfter) {
        throw new Error('Found a duplicate frequency: ' + frequency)
      }
    })
  }
} catch (error) {
